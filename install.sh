...
sudo cp conf/site1ssl.com.conf /etc/apache2/sites-available/

ip=sudo ip addr | grep 'state UP' -A2 | tail -n1 | awk '{print $2}' | cut -f1 -d'/'


echo "$ip:443" >> sudo tee --append /etc/apache2/ports.conf
echo "$ip:80" >> sudo tee --append /etc/apache2/ports.conf
